    ...
    return (
        <div className='md:flex gap-10'>
            <div className='md:w-1/3 '>
                <Image width={350} height={400} alt={`imagen ${producto.nombre}`} src={`/assets/img/${producto.imagen}.jpg`} />
            </div>
            <div className='md:w-2/3'>
                <div className='flex justify-end'>
                    <button onClick={handleChangeModal} className="hover:bg-red-700 rounded-full p-2">
                ...
                <p className='text-5xl font-black mt-5 text-amber-500'>{formatearDinero(producto.precio)}</p>
                ...
                <button type='button' onClick={() => handleAgregarPedido({...producto, cantidad})}
                    className='bg-indigo-600 hover:bg-indigo-800 px-5 py-2 mt-5 text-white font-bold uppercase rounded-md'>
                    {edicion ? 'Añadir Producto' : 'Añadir al Pedido'}
                </button>
            </div>
        </div>
    )
